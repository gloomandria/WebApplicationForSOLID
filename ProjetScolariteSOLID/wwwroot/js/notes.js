/**
 * Gestion AJAX de la section Notes — DataTables server-side.
...
(function ($) {
    ...
    function noteValeurHtml(v) {
        var cls = parseFloat(v) >= 10 ? 'bg-success' : 'bg-danger';
        return '<span class="badge ' + cls + '">' + v + '</span>';
    }

    function initDt() {
        dt = $('#dt-notes').DataTable({
            serverSide: true,
            processing: true,
            ajax: {
                url: '/' + ctrl + '/DataJson',
            ...
            columns: [
                { data: 'etudiant' },
                { data: 'matiere' },
                { data: 'valeur', className: 'text-center', render: function (v) { return noteValeurHtml(v); } },
                { data: 'typeEvaluation', render: function (v) { return v ? '<span class="badge bg-secondary">' + v + '</span>' : ''; } },
                { data: 'date' },
                { data: 'commentaire' },
